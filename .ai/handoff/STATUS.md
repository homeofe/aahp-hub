> Note (2026-07-26, claude-opus-5): Added .github/workflows/ci.yml, the first CI job in this repository that runs the application's own checks. Until now the only two workflows were AAHP Verify (handoff-state integrity) and Supply Chain Guard (dependency risk); neither compiles, type-checks or executes lib/ or app/, so a change that broke the Vitest suite still reported two green checks. The new job runs `npm run lint`, `npm test`, `npm run build` and `tsc --noEmit` on push to main and on every pull request. The type check runs after the build on purpose: Next.js generates the route helper types (PageProps and friends) into .next/types and next-env.d.ts during the build, and tsconfig.json includes those generated files, so a bare `tsc --noEmit` on a fresh checkout fails with "Cannot find name 'PageProps'" for a reason that has nothing to do with the code. All four steps were confirmed green on unmodified main (195 tests in 14 files, eslint clean, build clean). The gate was proven to bite: a scratch branch carrying the same workflow plus a deliberate regression (a strict top-level key allowlist in the manifest parser) turned the job red at the unit test step, run 30193587435. This job is the candidate for a required status check alongside AAHP Verify.
