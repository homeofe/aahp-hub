...
- 2026-07-24: security & ui: resolved security advisories (sharp ^0.35.3, js-yaml ^4.3.0, brace-expansion ^2.1.2 overrides, next ^16.2.11 bump; 0 open advisories), merged Dependabot PR #12 (actions/setup-node@v7), enabled main branch protection on GitHub repo, implemented Issue #8 (estate dependency posture & update cadence with lib/posture.ts, fixture tests lib/posture.test.ts, and /posture dashboard page), added turbopack.root and outputFileTracingExcludes to next.config.ts (0 build warnings), and redesigned WebUI with ProjectExplorer (Grid, Compact, and Ultra-dense Table views, multi-facet search/filters, pagination, localStorage options).
